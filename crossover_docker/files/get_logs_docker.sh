...
docker exec -it $name_apache_container bash -c "cp -r $apache_logs_docker $apache_dest_docker/$backups_logs/$fecha/access_log"
...
aws s3 sync /var/www/html/backups_logs s3://$bucket_name/
